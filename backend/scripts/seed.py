...
PROJECTS: list[dict] = [
    {
        ...
        "body_en": (
            ...
            "plate.\n\n"
            "## System Overview\n\n"
            "![System architecture — browser → Caddy → FastAPI → LangGraph, ChromaDB, LLM providers and PostgreSQL](/system.png)\n\n"
            ...
            "intent in a single LLM call, runs hybrid retrieval over an internal knowledge base (51 "
            "construction documents, ~1,375 chunks), and grounds its answers in retrieved passages with "
            "visible sources. It also understands **images** (upload a site photo or a scanned form and ask "
            "about it), answers questions over **documents you attach mid-chat** (PDF/Word/PPT, chunked into a "
            "conversation-private vector index), accepts **voice input**, and can find/download static "
            "checklists, fill them out conversationally, or generate brand-new structured forms on the fly\n"
            ...
            "- **PostgreSQL for all persistence (migrated from SQLite)** — the project shipped on SQLite for "
            "zero ops overhead, but once it had real concurrent users and real schema migrations, SQLite on "
            "Windows became a liability (see below); everything now lives in PostgreSQL — three logical "
            "databases (`kb_app`, `kb_langgraph`, `kb_search`) on one server — which closed an entire category "
            "of Windows/SQLite failure modes at the architecture level\n"
            "- **Gemini 3.5 Flash for vision and speech, OpenAI gpt-5.4 for the main reasoning** — one "
            "`get_llm()` factory swaps the provider per node from config, so image OCR/description, voice "
            "input, and the chat responder each run on the model that fits them best\n"
            ...
            "![CRAG — retriever → context builder → grader → query rewriter, capped at two retries](/CRAG.png)\n\n"
            ...
            "**3. One LLM call decides what every turn is.** The earlier design was a brittle keyword-plus-LLM "
            "hybrid that misclassified short messages and grew an ever-expanding keyword list. Now each turn "
            "makes a single structured-output LLM call that picks one of six intents — Q&A, static-form "
            "download, static-form fill, dynamic-form generate, form continuation, dynamic-form export — and "
            "decides whether retrieval is even needed. Plain Python then guard-rails the result: a form id the "
            "model hallucinated, a continuation with no prior form, an export with nothing to export all fall "
            "back safely to Q&A, so a confident-but-wrong classification can never derail the rest of the "
            "graph.\n\n"
            "![Unified intent — one structured LLM call, six intents, with code-level guard rails](/intent_workflow.png)\n\n"
            ...
            "an isolated worktree first. This incident is also the single biggest reason I later migrated the "
            "whole system from SQLite to PostgreSQL.\n"
            ...
            "users and real schema migrations. **Migrating the whole system to PostgreSQL** (now done — three "
            "databases on one server) removed an entire category of Windows/SQLite-specific failure modes I'd "
            "hit more than once.\n\n"
            "**Image input is now live** in the knowledge assistant — a vision-capable model in the loop, so "
            "staff can upload a site photo, a scanned form, or a diagram and ask questions grounded in *that* "
            "image, with multi-turn continuity so follow-up questions still know which image you meant. The "
            "trickiest design decision — dropping the carried-over image the moment a new document is "
            "uploaded, so \"summarize this file\" isn't hijacked by a stale photo — came straight out of a "
            "real bug.\n\n"
        ...
        "body_zh": (
            ...
            "要有確定性的檢查機制兜底。\n\n"
            "## 系統概觀\n\n"
            "![系統架構 — 瀏覽器 → Caddy → FastAPI → LangGraph、ChromaDB、LLM 與 PostgreSQL](/system.png)\n\n"
            "整個系統是一個 FastAPI 後端搭配一個 Next.js 前端，共用一次登入、服務兩個模組：\n\n"
            "- **模組 A — 知識助理（RAG）**：一條 15 個節點的 LangGraph 對話流程，先用單一 LLM call 判斷使用者"
            "意圖，再從內部知識庫（51 份營造文件、約 1,375 個切片）做 Hybrid 檢索、附上可追溯來源回答問題；"
            "同時支援**圖片理解**（上傳工地照片或表單截圖直接提問）、**聊天中附檔問答**（PDF／Word／PPT 即時"
            "切塊進「對話專屬」向量索引）、**語音輸入**，也能依需求找出、下載、用對話方式逐步代填既有檢核表，"
            "或對沒有對應範本的需求即時生成全新的結構化表格\n"
            ...
            "融合，不需要手動調整混合權重\n"
            "- **全面改用 PostgreSQL（已從 SQLite 完成遷移）** —— 一開始選 SQLite 是為了零維運成本、快速上線，"
            "但一旦遇上真實的並發使用者與 schema migration，SQLite 在 Windows 上就成了負擔（詳見下文）；現在"
            "所有持久化都落在 PostgreSQL —— 同一台 server 上的三個資料庫（`kb_app`、`kb_langgraph`、"
            "`kb_search`）—— 從架構面根除了一整類 Windows／SQLite 特有的故障模式\n"
            "- **Gemini 3.5 Flash 負責視覺與語音、OpenAI gpt-5.4 負責主推理** —— 由一個 `get_llm()` factory "
            "依設定逐節點切換 provider，讓圖片 OCR／描述、語音轉文字、對話生成各自跑在最合適的模型上\n"
            ...
            "問題」之間的關鍵差異。\n\n"
            "![CRAG —— retriever → context_builder → grader → query rewriter，最多重試兩次](/CRAG.png)\n\n"
            ...
            "污染自己的真相來源。\n\n"
            "**3. 用單一 LLM call 決定每一輪的意圖。** 舊版是「keyword fast-path + LLM」混合，短訊息容易字面"
            "誤判、keyword 集合又無限膨脹。現在每一輪都只打一次結構化輸出的 LLM call，從六種意圖中選一種 —— "
            "問答、靜態表單下載、靜態表單代填、動態表單生成、表單延續、動態表單匯出 —— 並順帶決定「這一輪到底"
            "需不需要檢索」。接著由純 Python 做越界防護：模型幻覺出的表單 id、沒有前一張表卻說要延續、沒東西"
            "可匯出卻要匯出，全都安全退回問答 —— 讓一個「很有自信但其實錯」的分類，永遠不會帶歪後面整張 graph。\n\n"
            "![Unified intent —— 單一結構化 LLM call、六種意圖，加上 code 層越界防護](/intent_workflow.png)\n\n"
            "## 遇到的問題與解決方法\n\n"
            ...
            "並先在隔離的 worktree 跑過一輪。這次事故，也是後來把整個系統從 SQLite 遷移到 PostgreSQL 最直接的原因。\n"
            ...
            "一旦遇上真實的並發使用者與真實的 schema migration，隨時可能變成錯誤的選擇。**把整個系統遷移到 "
            "PostgreSQL**（現已完成，同一台 server 上三個資料庫）一次解掉了我已經踩過不只一次的整類 "
            "Windows / SQLite 特有問題。\n\n"
            "知識助理的**圖片輸入能力現已正式上線**：接入具備視覺理解的模型，讓同仁可以上傳工地照片、表單"
            "掃描檔或圖面，針對「這張圖」提問，並支援多輪對話下的圖片延續性，讓追問時系統仍知道你指的是哪"
            "張圖。其中最棘手的一個設計決策 —— 一旦使用者上傳新文件就清掉沿用中的舊圖，避免「總結這份檔案」"
            "被前一張舊圖綁架 —— 正是從一個實際踩過的 bug 來的。\n\n"
...
POSTS: list[dict] = [
    {
        ...
        "body_en": (
            ...
            "serves a small JSON API the frontend calls, and now also runs the RAG chat assistant below.\n"
            ...
            "in that table.\n"
            "- **doc_chunks** — a pgvector table of embedded content chunks that now powers the chat "
            "assistant. Each chunk also stores a content hash, which is the key to the sync trick below.\n\n"
            "## Meet the assistant\n\n"
            "There's now a small chat widget on the site, built with LangGraph and Gemini. Ask it something "
            "about me, a project, or a post, and it streams back a grounded answer with links to the right "
            "page — in whichever language (EN / 中) you're asking in.\n\n"
            "Under the hood it's a tiny 3-node graph:\n\n"
            "1. **Condense** — rewrite a follow-up question into a standalone search query, resolving "
            "references like “that one” using the conversation so far.\n"
            "2. **Retrieve** — embed the query and pull the closest chunks from `doc_chunks` via pgvector's "
            "cosine search, scoped to the current language.\n"
            "3. **Generate** — stream an answer grounded only in those chunks, citing sources as Markdown "
            "links. If the context doesn't cover the question, it says so instead of guessing.\n\n"
            "## Keeping the index in sync\n\n"
            "The trickiest part of any RAG setup is keeping the vector index honest as content changes. "
            "`doc_chunks` now syncs incrementally: every source (each project, post, and the profile blurb, "
            "in each language) is hashed, and only sources whose hash changed get re-chunked and "
            "re-embedded — unchanged ones are left alone. Anything removed or unpublished has its chunks "
            "deleted too. This runs after seeding and as a background task when the API starts, so the "
            "assistant's knowledge stays current without me remembering to run anything by hand. A "
            "`--full` flag is still there if I ever need to force a complete rebuild (say, after changing "
            "the chunker or embedding model).\n\n"
            "## Why this architecture\n\n"
            "A few deliberate choices, holding up well so far:\n\n"
            "- **A separate Python backend.** Python is my home turf (FastAPI, SQLAlchemy), and it turned "
            "out to be the natural home for the LangGraph assistant above. A clean API boundary keeps the "
            "frontend simple.\n"
            "- **Next.js on Vercel.** Great developer experience, server rendering + ISR for speed and SEO, "
            "and effortless deploys.\n"
            "- **Supabase + pgvector.** One database for content, embeddings, and now content hashes — the "
            "assistant and the incremental sync both live on top of it with zero extra infrastructure.\n"
            "- **Content as data.** Storing projects and posts in the DB (instead of hardcoding them) lets "
            "me add or edit things without redeploying, and the assistant picks up the changes on its own.\n\n"
            "## What's next\n\n"
            "With the assistant live, the next focus is tuning retrieval quality as more projects and posts "
            "land, and writing more of these notes as I go.\n"
        ),
        "body_zh": (
            "## 為什麼做這個網站\n\n"
            ...
            "JSON API 給前端呼叫，現在也負責跑下面這個 RAG 聊天助手。\n"
            ...
            "- **doc_chunks** — 一張存放內容切片與向量的 pgvector 表，現在正式撐起聊天助手。每一筆還存了一個"
            "內容雜湊值（content hash），這就是下面同步機制的關鍵。\n\n"
            "## 認識這個助手\n\n"
            "網站上現在有一個小聊天小工具，用 LangGraph 與 Gemini 打造。問它任何關於我、專案或文章的問題，"
            "它會即時串流回答，並附上連到對應頁面的連結——而且會用你提問的語言（中／英）回覆。\n\n"
            "背後是一個三節點的小型 graph：\n\n"
            "1. **Condense（改寫問題）** — 把追問改寫成一句獨立的搜尋查詢，並用對話歷史解析「那個」之類的代稱。\n"
            "2. **Retrieve（檢索）** — 將查詢轉成向量，透過 pgvector 的 cosine 相似度從 `doc_chunks` 撈出最相關"
            "的切片，並限定在同一語言。\n"
            "3. **Generate（生成）** — 只根據這些切片串流產生回答，並以 Markdown 連結引用來源。如果內容沒有"
            "涵蓋這個問題，它會老實說不確定，而不是亂猜。\n\n"
            "## 讓索引保持同步\n\n"
            "任何 RAG 系統最麻煩的部分，就是讓向量索引跟著內容變動保持正確。`doc_chunks` 現在會做增量同步："
            "每個來源（每篇專案、文章、以及個人簡介，每種語言各算一份）都會被算出雜湊值，只有雜湊值改變的來源"
            "才會重新切片、重新 embedding——沒變的就維持不動；被刪除或下架的內容，對應的切片也會一併移除。"
            "這個流程會在 seed 之後自動執行，也會在 API 啟動時以背景任務跑一次，讓助手的知識保持最新，不用"
            "我手動跑指令。如果以後改了切割邏輯或 embedding 模型，還是可以用 `--full` 參數強制全部重建。\n\n"
            "## 為什麼選這樣的架構\n\n"
            "幾個刻意的選擇，到目前為止都還挺站得住腳：\n\n"
            "- **獨立的 Python 後端。** Python 是我最熟的主場（FastAPI、SQLAlchemy），後來也證明是放上面"
            "那個 LangGraph 助手最自然的地方。清楚的 API 邊界也讓前端保持單純。\n"
            "- **Next.js + Vercel。** 開發體驗好、有 SSR + ISR 兼顧速度與 SEO，部署也很省事。\n"
            "- **Supabase + pgvector。** 同一個資料庫同時放內容、向量，現在還多了內容雜湊——聊天助手與"
            "增量同步都直接架在上面，沒有多加任何基礎設施。\n"
            "- **內容即資料。** 把專案與文章存在資料庫（而不是寫死），不重新部署也能新增／修改，助手也會"
            "自動跟上這些變化。\n\n"
            "## 接下來\n\n"
            "助手上線之後，接下來的重點會放在隨著專案與文章增加持續調整檢索品質，並繼續在這裡記錄這些過程。\n"
        ),
